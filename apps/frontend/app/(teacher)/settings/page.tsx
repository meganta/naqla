...
import { getSettings, updateSettings, api } from "@/lib/api-client";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (!token) return;
    api.google.getStatus(token)
      .then((s: any) => setGoogleConnected(s.connected))
      .catch(() => {});
    getSettings(token)
      .then((s: any) => {
        setData({
  ...
  const handleGoogleConnect = async () => {
    ...
    try {
      const { auth_url } = await api.google.getAuthUrl(token);
      window.location.href = auth_url;
    } catch (e: any) {
      setMessage({ text: (e as Error).message || "حدث خطأ", error: true });
    ...
    <div dir="rtl" className="max-w-2xl mx-auto">
      ...
      <Section title="قناة يوتيوب">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <p className="text-xs font-medium text-gray-600 mb-2">ربط حساب Google</p>
          <p className="text-xs text-gray-400 mb-3">
          ...
          {googleConnected ? (
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50"
            >
              ...
              {googleLoading ? "جاري التوجيه..." : "ربط حساب Google"}
            </button>
          )}
        </div>
      </Section>
